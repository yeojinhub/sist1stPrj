...
public class AdminCourseView extends JFrame{
	...
	private AdminCourseVO selectedCourInfo;
	
	public AdminCourseView(AdminMainView amView) {
		this.amView=amView;
//		this.courseList = amView.getCourseList();
		//디자인(색깔,글씨) 설정
        ...
        String[] strAdminCourseTableTitle = {
            "과정번호","기수", "과정명", "진행상태", "담당강사", "시작일", "종료일"
        };

        //Table Component 생성
        dftmAdminCourseTableModel = new DefaultTableModel(null, strAdminCourseTableTitle);
		...
		// 교번 숨키기
		jtAdminCourseTable.getColumnModel().getColumn(0).setMinWidth(0); // 최소 너비
		jtAdminCourseTable.getColumnModel().getColumn(0).setMaxWidth(0); // 최대 너비
		jtAdminCourseTable.getColumnModel().getColumn(0).setWidth(0); // 최대 너비
	...
	public AdminCourseVO getSelectedCourInfo() {
		return selectedCourInfo;
	}

	public void setSelectedCourInfo(AdminCourseVO selectedCourInfo) {
		this.selectedCourInfo = selectedCourInfo;
	}
